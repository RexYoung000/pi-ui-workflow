...
**重要前置检查**：如果 $@ 为空（用户只输入了 `/ui-workflow` 没有带任何描述），**先不要进入任何阶段**。立即用 `ask_user_question` 问用户：

> **「你想设计什么产品？请简单描述一下。」**
>
> （开放文本问题，让用户用自己的话描述要做的产品、功能或页面。）

拿到用户描述后，把它当作 $@，再继续下面的流程。
...
用 `ui_workflow_subagent` 工具的 **single 模式**调用 `ui-research-analyst`：
...
用 `ui_workflow_subagent` 工具 **parallel 模式**，每个 task 至少包含：执行模式 + need 完整报告 + research 完整报告。
...
深度模式可以使用 `ui_workflow_subagent` 的 single 模式逐个调用，或在不破坏上游依赖的前提下做小范围并行。每一步完成后都要给用户一条等待反馈，例如：
...
- 📊 跑了哪些 Agent、核心结论
- ⚠️ 待确认问题（来自 need 报告）
- 💰 成本/耗时体感：说明本次属于快速 / 标准 / 深度模式，以及大致等待和消耗级别，不承诺精确 token 统计
- 💡 下一步建议
